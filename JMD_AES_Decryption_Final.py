...
Round=ks(Key)
step=iark(step(ip),Round[10])
step=isr(step)
step=ibs(step)
i=0
j=9
while i<9:
    step=iark(step,Round[j])
    step=imc(step)
    step=isr(step)
    step=ibs(step)
    i+=1
    j-=1
step=iark(step,Round[0])
print 'The Plain text:',step
